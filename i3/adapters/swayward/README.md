...
resulting KDL to swayward. Translation failures abort the test instead of
silently running with defaults. The runner mounts a private `/tmp`, including
both X11 sockets and display lock files, so parallel satellite instances cannot
collide with each other or the host.
