...
No file under `i3/t/` changed. The runner reads the matching files from
...
- `141-resize.t` assertion 83 passed accidentally because the helper compared
  two undefined wrapper-child rectangles. the imported swayward result already records
...
gate. Do not update a result file from these aggregate rows.
