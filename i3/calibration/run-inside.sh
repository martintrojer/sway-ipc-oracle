...
cp "$REPO/i3/calibration/SocketActivation.pm" "$STAGE/SocketActivation.pm"
mkdir -p "$STAGE/swaycal"
cp "$REPO/i3/calibration/swaycal/Shim.pm" "$STAGE/swaycal/Shim.pm"
...
while read -r t; do
  ...
  systemd-run --user --scope -q -p MemoryMax=$MEMMAX -p MemorySwapMax=0 \
    ...
    unshare -rm "$REPO/i3/calibration/run-one.sh" \
    > "$OUT/$t.tap" 2>&1
  echo "EXIT=$? $t"
